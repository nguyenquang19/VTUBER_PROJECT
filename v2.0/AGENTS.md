...
- Đọc `docs/V1_BASELINE.md` trước mọi task để phân biệt baseline lịch sử với capability phát sinh sau đó;
  đọc `docs/MAI_V2_SYSTEM_SPEC.md` để xác định trạng thái implementation hiện tại.
...
- `docs/V1_BASELINE.md` — release baseline bất biến, invariant và version policy.
- `docs/MAI_V2_SYSTEM_SPEC.md` — nguồn sự thật duy nhất về runtime, component, pipeline, config,
  storage, operations, security, testing, tiến độ và known gaps hiện tại.
- `docs/README.md` — mục lục tối giản và quy tắc giữ tài liệu sạch.

Không tạo hoặc tìm tài liệu riêng theo phase/component. Các tài liệu runtime cũ đã được chuẩn hóa và gộp
vào System Spec. Blueprint V2 là source of truth cho scope và thứ tự migration; code/interfaces/tests/config
cùng System Spec là source of truth cho behavior đã triển khai.
...
Conversation continuity, Thread Engine và mọi subsystem khác dùng mục tương ứng trong
`docs/MAI_V2_SYSTEM_SPEC.md`; không tạo lại kế hoạch/architecture riêng đã được gộp.
...
- Không đặt tên file test theo phase/milestone (`_m*`, `phase*`, `m8_...`); đặt theo component/hành vi.
  Không thêm test-runner ad-hoc song song với `pytest` + marker.
...
2. Đọc `docs/V1_BASELINE.md`, `docs/MAI_V2_SYSTEM_SPEC.md`, code/config và impacted tests.
