...
function renderSystem(snapshot) {
  ...
  fillDl("system-tts", [["Turns", tts.turns_total], ["Subtitle fallback", tts.subtitle_fallback_total], ["Played chunks", (tts.player || {}).chunks_played], ["Dropped chunks", (tts.player || {}).chunks_dropped]]);  const world = snapshot.world || {};
  const worldSnapshot = world.snapshot || {};
  const worldMetrics = world.metrics || {};
  const worldDomains = ["stream", "social", "call", "media", "physical", "game"];
  const worldEntries = worldDomains.reduce((count, domain) => count + Object.keys(worldSnapshot[domain] || {}).length, 0);
  const worldEvents = worldMetrics.world_model_events || {};
  const worldAccepted = Object.entries(worldEvents).reduce((count, [key, value]) => count + (key.startsWith("accepted:") ? Number(value || 0) : 0), 0);
  const worldRejected = Object.entries(worldEvents).reduce((count, [key, value]) => count + (key.startsWith("rejected:") ? Number(value || 0) : 0), 0);
  fillDl("system-world", [["Mode", worldMetrics.world_model_enabled ? "Shadow enabled" : "Unavailable / disabled"], ["Fresh entries", world.snapshot ? worldEntries : "—"], ["Accepted", world.snapshot ? worldAccepted : "—"], ["Rejected", world.snapshot ? worldRejected : "—"], ["Stale evicted", world.snapshot ? worldMetrics.world_model_evicted_total : "—"]]);
  const selfModel = snapshot.self || {};
  const selfSnapshot = selfModel.snapshot || {};
  const selfMetrics = selfModel.metrics || {};
  fillDl("system-self", [
    ["Mode", selfModel.snapshot ? (selfMetrics.self_model_enabled ? "Projection active" : "Disabled") : "Unavailable"],
    ["Speaking", selfModel.snapshot ? (selfSnapshot.speaking ? "Yes" : "No") : "—"],
    ["Busy", selfModel.snapshot ? (selfSnapshot.busy ? "Yes" : "No") : "—"],
    ["Degraded", selfModel.snapshot ? (selfSnapshot.degraded ? "Yes" : "No") : "—"],
    ["Action", selfSnapshot.current_action_id],
    ["Goal", selfSnapshot.active_goal_id],
    ["Thread", selfSnapshot.focused_thread_id],
    ["Recent actions", (selfSnapshot.recent_action_ids || []).join(", ")],
  ]);
  const capabilityHost = byId("system-capabilities");
  const capabilitySnapshot = (snapshot.capabilities || {}).snapshot || {};
  setListEmpty(capabilityHost, "Chưa có capability snapshot.");
  (capabilitySnapshot.capabilities || []).forEach((entry) => {
    const capability = entry.capability || {};
    const availability = entry.availability || {};
    const tone = availability.available ? "ready" : "failed";
    const mode = entry.mock_only ? "mock-only" : "declared";
    addStackItem(
      capabilityHost,
      `${capability.capability_id || "capability"} · ${availability.available ? "AVAILABLE" : "BLOCKED"}`,
      `${availability.reason_code || "unknown"} · ${mode}`,
      tone,
    );
  });
  const mockHost = byId("system-action-mock");
  const mockSnapshot = ((snapshot.action_mock || {}).snapshot || {});
  setListEmpty(mockHost, "Chưa có mock action.");
  (mockSnapshot.recent || []).forEach((result) => {
    addStackItem(
      mockHost,
      `${result.action_id || "action"} · ${result.verified ? "VERIFIED" : result.status || "unknown"}`,
      result.error_code || result.verification_source || "mock-only",
      result.verified ? "ready" : "failed",
    );
  });
}
