...
| `capabilities.yaml` | Capability declarations, permission grants, health/availability preconditions, and Phase 5 mock-action bounds |
...
`action_transactions`, `decision_records`, `operator_dashboard_v2`, `proactive_hosting`, `world_model_shadow`, `self_model_projection`, `capability_registry`, `action_mock_closed_loop`,
`self_talk_planner`, `behavior_library`, `natural_timing`, `world_model_shadow`, `self_model_projection`, `capability_registry`, `action_mock_closed_loop`, `director_v2_shadow`, `relationship_memory`, `evaluation_harness`,
...
- `system.yaml::app.version` là product version duy nhất (`1.0.0` ở baseline, `1.4.3` hiện tại).
