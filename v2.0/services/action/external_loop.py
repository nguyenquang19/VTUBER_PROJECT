...
from interfaces.action_execution import ActionVerifier, GeneralActionService, VerificationResult
...
class ExternalActionConfig:
    ...
    def from_loader(cls, loader: Any) -> "ExternalActionConfig":
        raw = loader.get("capabilities", "external_actions", None)
        obs = loader.get("capabilities", "external_actions.obs", None)
        if not isinstance(raw, Mapping) or not isinstance(obs, Mapping):
            raise ValueError("external_actions config must be a mapping")
        return cls(
