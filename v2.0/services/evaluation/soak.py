...
class SoakConfig:
    ...
    def from_loader(cls, loader: Any) -> "SoakConfig":
        prefix = "evaluation.operations.soak."
        return cls(
            duration_s=float(loader.get("evaluation", prefix + "duration_s", 7200)),
            sample_interval_s=float(loader.get("evaluation", prefix + "sample_interval_s", 5)),
            input_rate_hz=float(loader.get("evaluation", prefix + "input_rate_hz", 20)),
            queue_capacity=int(loader.get("evaluation", prefix + "queue_capacity", 256)),
            progress_timeout_s=float(loader.get("evaluation", prefix + "progress_timeout_s", 30)),
            latency_sample_max=int(loader.get("evaluation", prefix + "latency_sample_max", 20000)),
            max_memory_growth_mb=float(loader.get("evaluation", prefix + "max_memory_growth_mb", 128)),
            max_queue_growth=int(loader.get("evaluation", prefix + "max_queue_growth", 20)),
            max_error_rate=float(loader.get("evaluation", prefix + "max_error_rate", 0.01)),
            latency_p95_budget_ms=float(loader.get(
                "evaluation", prefix + "latency_p95_budget_ms", 1500,
            )),
            report_file=Path(loader.get(
                "evaluation", prefix + "report_file",
